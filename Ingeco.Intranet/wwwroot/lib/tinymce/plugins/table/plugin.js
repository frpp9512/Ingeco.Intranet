...
(function (domGlobals) {
    ...
    var detailnew = Immutable('element', 'rowspan', 'colspan', 'isPost');
    var extended = Immutable('element', 'rowspan', 'colspan', 'row', 'column');
    var rowdata = Immutable('element', 'cells', 'section');
    var elementnew = Immutable('element', 'isPost');
    var rowdatanew = Immutable('element', 'cells', 'section', 'isPost');
      ...
      return map(grid, function (row, ri) {
        var details = bind(row.cells(), function (cell, ci) {
          if (seen[ri][ci] === false) {
            ...
            return [detailnew(cell.element(), result.rowspan(), result.colspan(), cell.isPost())];
          } else {
            return [];
          }
        });
    ...
    var toGrid = function (warehouse, generators, isPost) {
      var grid = [];
      for (var i = 0; i < warehouse.grid().rows(); i++) {
        var rowCells = [];
        for (var j = 0; j < warehouse.grid().columns(); j++) {
          var element = Warehouse.getAt(warehouse, i, j).map(function (item) {
            return elementnew(item.element(), isPost);
          }).getOrThunk(function () {
    ...
    var deriveRows = function (rendered, generators) {
      var findRow = function (details) {
        var rowOfCells = findMap(details, function (detail) {
          return parent(detail.element()).map(function (row) {
            var isPost = parent(row).isNone();
            return elementnew(row, isPost);
          });
        });
      ...
      return map(rendered, function (details) {
        ...
        return rowdatanew(row.element(), details.details(), details.section(), row.isPost());
      });
    };
    ...
    var insert = function (index, gridA, gridB, generator, comparator) {
      ...
      var fittedPostGrid = Fitment.tailor(gridB, delta, generator);
      var secondDelta = Fitment.measureWidth(gridA, fittedPostGrid);
      var fittedOldGrid = Fitment.tailor(gridA, secondDelta, generator);
      return fittedOldGrid.slice(0, index).concat(fittedPostGrid).concat(fittedOldGrid.slice(index, fittedOldGrid.length));
    };
    var TableMerge = {
      merge: merge$1,
      insert: insert
      ...
      return function (generators) {
        ...
        var replaceOrInit = function (element, comparator) {
          return find$1(element, comparator).fold(function () {
            ...
            return comparator(element, p.item) ? p.sub : makePost(element);
          });
        };
        return {
    ...
    var firePostRow = function (editor, row) {
      return editor.fire('newrow', { node: row });
    };
    var firePostCell = function (editor, cell) {
        ...
        return function (table, target) {
          ...
          return guard(table) ? operation(wire, table, target, generators, direction).bind(function (result) {
            each(result.newRows(), function (row) {
              firePostRow(editor, row.dom());
            });
            each(result.newCells(), function (cell) {
              firePostCell(editor, cell.dom());
            });
    ...
    var getItems$2 = function (editor, hasClasses, insertPostTable) {
      var rowColCountItems = !insertPostTable ? [] : [
        {
          type: 'input',
          name: 'cols',
          label: 'Cols',
    ...
    var open$2 = function (editor, insertPostTable) {
      ...
      var generalPanel = {
        type: 'grid',
        columns: 2,
        items: TableDialogGeneralTab.getItems(editor, hasClasses, insertPostTable)
      };
    ...
    var getPostRowCursorPosition = function (editor, table) {
    ...
    var go = function (editor, isRoot, cell, actions, lazyWire) {
      return cell.fold(Option.none, Option.none, function (current, next) {
        ...
        return TableLookup.table(current, isRoot).bind(function (table) {
          ...
          return getPostRowCursorPosition(editor, table);
        });
      });
    };
    var rootElements = [
