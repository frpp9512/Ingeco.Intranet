...
(function (domGlobals) {
    ...
    function isWordContent(content) {
      return /<font face="Times Post Roman"|class="?Mso|style="[^"]*\bmso-|style='[^'']*\bmso-|w:WordDocument/i.test(content) || /class="OutlineElement/.test(content) || /id="?docs\-internal\-guid\-/.test(content);
    }
    ...
    var isPostLineChar = function (c) {
      return c === '\n' || c === '\r';
    };
    var isPostline = function (text, idx) {
      return idx < text.length && idx >= 0 ? isPostLineChar(text[idx]) : false;
    };
    var normalizeWhitespace = function (text) {
      var result = foldl(text, function (acc, c) {
        if (isCollapsibleWhitespace(c) || c === nbsp) {
          if (acc.pcIsSpace || acc.str === '' || acc.str.length === text.length - 1 || isPostline(text, acc.str.length + 1)) {
            return {
              pcIsSpace: false,
              str: acc.str + nbsp
            };
    ...
    var pasteText = function (editor, text) {
      ...
      var html = Postlines.convert(normalizedText, editor.settings.forced_root_block, editor.settings.forced_root_block_attrs);
    ...
    var registerEventHandlers = function (editor, pasteBin, pasteFormat) {
      ...
      function insertClipboardContent(clipboardContent, isKeyBoardPaste, plainTextMode, internal) {
        ...
        pasteBin.remove();
        isPlainTextHtml = internal === false && Postlines.isPlainText(content);
